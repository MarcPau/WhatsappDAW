...
CREATE TABLE grupos (
    id INT AUTO_INCREMENT PRIMARY KEY,    -- Identificador único del grupo
    grupo VARCHAR(255) NOT NULL           -- Nombre del grupo
);

CREATE TABLE mensajes (
    id INT AUTO_INCREMENT PRIMARY KEY,            -- Identificador único del mensaje
    mensaje TEXT NOT NULL,                        -- Contenido del mensaje
    id_emisor INT NOT NULL,                       -- ID del emisor del mensaje
    id_receptor INT,                              -- ID del receptor (NULL si es para un grupo)
    id_grupo INT,                                 -- ID del grupo (NULL si es un mensaje privado)
    estado ENUM('enviado', 'entregado', 'leido') DEFAULT 'enviado', -- Estado del mensaje
    fecha_envio DATETIME DEFAULT CURRENT_TIMESTAMP, -- Fecha y hora del envío
    FOREIGN KEY (id_emisor) REFERENCES usuarisclase(id), -- Relación con tabla de usuarios
    FOREIGN KEY (id_receptor) REFERENCES usuarisclase(id), -- Relación con tabla de usuarios
    FOREIGN KEY (id_grupo) REFERENCES grupos(id)       -- Relación con tabla de grupos
);
...
CREATE TABLE grupo_usuarios (
    ...
    admin bool NOT NULL,                  -- Determina si un usuario es o no es admin de un grupo
    PRIMARY KEY (id_grupo, id_usuario),   -- Llave primaria compuesta
    FOREIGN KEY (id_grupo) REFERENCES grupos(id), -- Relación con tabla grupos
    FOREIGN KEY (id_usuario) REFERENCES usuarisclase(id) -- Relación con tabla usuarios
);

CREATE TABLE estado_mensajes_grupo (
    ...
    FOREIGN KEY (id_usuario) REFERENCES usuarisclase(id)
);
